...
- **Проверено 2026-06-29:** `node --check` (theme + site-code + section-bg + ai-pipeline + main); `npm run test:e2e:editor-v2` 41/41 зелёные после каждого выноса (оверлей-тест покрывает bgInspector+liveOverlay+command-gesture; apply/preview/cancel-тесты покрывают AI pipeline).
