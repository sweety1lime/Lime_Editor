...
- **⚠️ Латентный баг (предсуществующий, НЕ от рефактора):** в `bgInspector` режим фона «Картинка» зовёт несуществующий top-level `seg(...)` → переключение фона секции на изображение падает (ReferenceError). Тесты не покрывают (оверлей-путь идёт в дефолтном режиме). При выносе поведение сохранено как есть (фантом не инъектился). Завести отдельным фиксом: построить сегмент-контрол для backgroundSize/backgroundPosition.
