...
- **Как:** managed-Postgres с авто-бэкапом, либо `pg_dump` по cron в отдельном контейнере compose (хранить off-site / в объектном сторадже, retention 7–30 дней). Медиа — версионирование бакета (после 2.2) или `rsync`-снапшоты тома. Раз в квартал — учебное восстановление из бэкапа.
...
- **Как:** `Sentry.AspNetCore` + DSN через env (как у AI-ключа). Serilog sink в агрегатор (Seq self-hosted / Grafana Loki). Correlation-id middleware (request-id в каждый лог и в ответ). Алерты на 5xx и на падение `/health`.
...
### 1.3 🔜 Email-подтверждение (включить enforcement)
- **Что:** перекрыть регистрацию на чужие адреса и спам-аккаунты.
- **Как:** подключить боевой SMTP (env `SMTP_*` — код Фазы 3 уже готов), слать confirm-письмо при регистрации, затем `options.SignIn.RequireConfirmedAccount = true` в `Startup`. До подключения SMTP оставить `false`.
...
- **Что:** `wwwroot/js/lime/lime-doc-editor.js` (было 6285 строк; сейчас 5731) и `HomeController` (CRUD сайтов + профиль + auth в одном).
...
### 4.3 🧹 Мелочи
- **Playwright auth-флоу** — прогнать sign-up/sign-in/forgot-password в браузере (правки M0 задели auth-вью и политику паролей). Нужен поднятый dev-Postgres. *Когда:* перед мерджем накопленных изменений.
